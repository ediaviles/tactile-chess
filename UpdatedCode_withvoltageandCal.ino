...
void loop() {
    ...
    if(!scanned){
      preValues[binaryCounter][groupCounter] = analogRead(ANALOG_PIN) * (5.0 / 1023);
      for(int r = 0; r < 8;r++){
        for(int c = 0; c< 8;c++){
          float voltage = scannedMatrix[r][c];
          if(min_volatage == -1 || max_voltage == -1){
            min_voltage = voltage;
            max_voltage = voltage;
